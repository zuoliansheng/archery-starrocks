...
def query(db_name=None, sql="", limit_num=0, close_conn=True, **kwargs):

    """返回 ResultSet"""
    result_set = ResultSet(full_sql=sql)
    try:
        conn = pymysql.connect(host="127.0.0.1",port=9030,user='root',password='',connect_timeout=10)
        cursor = conn.cursor()
        cursor.execute(sql)
        if int(limit_num) > 0:
...
def main():
    ...
    # result = query(db_name=db_name, sql=sql)
    # print(result.rows[0])
    # result.rows[0] = ((tb_name,) , (result.rows[0][1],))
    # print(result)
